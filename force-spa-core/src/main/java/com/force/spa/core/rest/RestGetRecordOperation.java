...
class RestGetRecordOperation<T> extends AbstractRestRecordOperation<T> implements GetRecordOperation<T> {
    ...
    public RestGetRecordOperation(String id, Class<T> recordClass, FieldLevelSecurityFilter fieldLevelSecurityFilter) {
        if (id == null)
    ...
    public void start(RestConnector connector, final ObjectMappingContext mappingContext) {
        ...
        String soql = new SoqlBuilder(descriptor, fieldLevelSecurityFilter).soqlTemplate(soqlTemplate).limit(1).build();
